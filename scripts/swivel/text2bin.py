...
#print('opt_vocab', opt_vocab)

def go(fhs):
  fmt = None
  with open(opt_vocab, 'w', encoding='utf_8') as vocab_out:
...
if args:
  print("merging files %s into output bin" % args)
  fhs = [open(filename, encoding='utf_8') for filename in args]
  go(fhs)
  for fh in fhs:
    fh.close()
else:
  print('no args, readin from stdin')
  go([sys.stdin])
