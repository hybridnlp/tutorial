...
flags.DEFINE_string('eval_word_prefix', '', 'Prefix to attach to the wordsim words, so that they match the vocabulary')
...
class Model(object):
  ...
  def __init__(self, input_base_path, hparams):
    ...
    l2_loss = loss_multiplier * tf.reduce_sum(
        0.5 * l2_confidence * tf.square(err) * count_is_nonzero)

    sigmoid_loss = loss_multiplier * tf.reduce_sum(
        tf.nn.softplus(err) * count_is_zero)

    with tf.name_scope('vector_regularization'):
      # try to make row and col embeddings similar to each other
      with tf.name_scope('row_embeddings_loss'):
        col_embedding_for_selected_rows = tf.nn.embedding_lookup(
          self.col_embedding, global_rows)
        row_embedding_loss = tf.nn.l2_loss(
          selected_rows - col_embedding_for_selected_rows)

      with tf.name_scope('col_embeddings_loss'):
        row_embedding_for_selected_cols = tf.nn.embedding_lookup(
          self.row_embedding, global_cols)
        col_embedding_loss = tf.nn.l2_loss(
          selected_cols - row_embedding_for_selected_cols)

      embedding_loss = tf.reduce_mean(
        row_embedding_loss) + tf.reduce_mean(col_embedding_loss)
      vector_regul_loss = hparams.row_col_vector_binding_factor * (
        embedding_loss)

    self.l2_loss_op = l2_loss
    self.sigmoid_loss_op = sigmoid_loss
    self.vector_regul_loss_op = vector_regul_loss
    self.loss_op = l2_loss + sigmoid_loss + vector_regul_loss
  ...
  def wordsim_eval_op(self, filename, word_prefix):
    ...
    with open(filename, 'r', encoding='utf8') as fh:
      ...
      print('wordsim', filename, 'has tuples', len(sims))
      actuals = list(map(float, sims))

    def word_ixs(words):
        return [self.row_word_to_ix.get(word_prefix + w, 0) for w in words]

    w1_ixs = word_ixs(word1s)
    w2_ixs = word_ixs(word2s)
    v1s_t = tf.nn.embedding_lookup(
        self.row_embedding, w1_ixs)

    v2s_t = tf.nn.embedding_lookup(
        self.row_embedding, w2_ixs)
    ...
    def _op(preds):
      print('preds shape', preds.shape, 'actuals', len(actuals))
